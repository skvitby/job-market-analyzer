...
3. **AC 3.3:** При предоставлении локального файла резюме (`profile/my_cv.md`) система сопоставляет навыки из отчёта с текстом резюме (учитывая синонимы и вариации формулировок, например `Postgres` / `PostgreSQL`) и подсвечивает пересечения и недостающие навыки в разделе отчёта «Сравнение с резюме»:
   ...
   - **Как сопоставляются (гибридно):** навыки словаря ищутся в резюме точно, с синонимами (как в AC 3.1); LLM из `llm_providers.cv_processing` (AC 1.5) дополняет навыки вне словаря и нестандартные формулировки одним запросом на весь список. Итоговый статус — лучший из двух.
   - **Защита от выдуманных совпадений:** LLM возвращает цитату из резюме; совпадение засчитывается, только если каждый фрагмент цитаты есть в тексте резюме.
