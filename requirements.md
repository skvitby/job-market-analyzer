...
2. **NFR-2 (Безопасность):** API-ключи (Anthropic API Key; `HH_CLIENT_ID`, `HH_CLIENT_SECRET`, `HH_APP_TOKEN`) должны храниться исключительно в файле `.env` и не попадать в публичный репозиторий Git.
...
2. **Накопление данных:** каждый запуск US-02 создаёт новый файл `raw_vacancies_{timestamp}.json`, а после первого запуска файлы содержат только вакансии, появившиеся с момента предыдущей выгрузки (AC 2.6). Поэтому `analyze` (US-03) должен объединять все файлы из `data/` с дедупликацией по `id`, а не брать только самый свежий (сейчас так описано в разделе 7). Механизм объединения и анализ трендов во времени — определить при реализации US-03.
...
4. **Полнота текста для Gap-анализа (US-03):** поиск HH возвращает только короткие сниппеты `requirement` / `responsibility` (~200 символов). Для полного анализа навыков нужны полные описания и `key_skills` через `GET /vacancies/{id}` — это отдельный запрос на каждую вакансию (при ~1000 вакансиях и паузе 3 сек — около часа). Решить при реализации US-03: анализировать сниппеты, догружать описания выборочно или кэшировать уже загруженные.
